...
class CityScapesDataset(Dataset):
    ...
    def __init__(self, csv_file, root_dir, transform=None):
        ...
        self._domain_to_ind = dict(
            zip(self.domains, range(len(self.domains))))  # Assigining a numerical index to each domain
    ...
    def __getitem__(self, idx):
        ...
        transformed = self.transform(image=image, bboxes=bboxes,
                                     class_labels=labels)  # Albumentations can transform images and boxes
...
tr_dataset = CityScapesDataset('../datasets/Annots/cityscapes_train.csv', root='../datasets/cityscapes_clear/train/',
                               transform=train_transform)
vl_dataset = CityScapesDataset('../datasets/Annots/cityscapes_val.csv', root='../datasets/cityscapes_clear/val/',
...
class DGFasterRCNN(LightningModule):
    def __init__(self, n_classes):
        ...
        self.detector = fasterrcnn.fasterrcnn_resnet50_fpn(min_size=600, max_size=1200, pretrained=True,
    ...
    def train_dataloader(self):
        ...
        return torch.utils.data.DataLoader(tr_dataset, batch_size=2, sampler=sample_indices, shuffle=False,
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.mode == 0:
            temp_loss = []
            detections = self.detector([imgs[0]], [targets[0]])
            for index in range(len(imgs)):


                ImgDA_scores = self.ImageDA(self.base_feat['0'])
                detections[0]['losses']['DA_img_loss'] = F.cross_entropy(ImgDA_scores,
                                                                         torch.unsqueeze(batch[3][index], 0))
                IDA_out = self.InsDA(self.box_features)
                detections[0]['losses']['DA_ins_loss'] = 0.1 * F.cross_entropy(IDA_out, batch[3][index].repeat(
                    IDA_out.shape[0]).long())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (torch.sum(
                unique_labels) > 0):  # Checking this will ensure deal with no_box conditions. But this will not handle where there are false detections. When TP=0, precision=0
            for label in unique_labels:
                indices_s = torch.where(labels[0] == label)
                indices_t = torch.where(preds[0]['labels'] == label)
                if len(indices_t[0]) > 0:
                    self.val_acc[label - 1] = self.val_acc[label - 1] + self.accuracy(boxes[0][indices_s[0]],
                                                                                      preds[0]['boxes'][indices_t[0]],
                                                                                      iou_threshold=0.5)
...
trainer = Trainer(gpus=1, progress_bar_refresh_rate=1, max_epochs=100, deterministic=False,
...
test_dataset = CityScapesDatasetTest('../datasets/Annots/cityscapes_foggy_val.csv',
                                     root='../datasets/cityscapes_foggy/val/', transform=valid_transform)
...
detector.to('cuda')
color_dict = {1: (0, 0, 255), 2: (0, 255, 0), 3: (255, 0, 0), 4: (255, 255, 0), 5: (0, 255, 255), 6: (255, 0, 255),
              7: (0, 0, 150), 8: (0, 150, 0)}
...
for index, data_sample in enumerate(test_dataset):
    ...
    if (torch.sum(
        ...
        for label in unique_labels:
            ...
            indices_t = torch.where(preds[0]['labels'] == label)
            if len(indices_t[0]) > 0:
                val_acc[label - 1] = val_acc[label - 1] + acc_new(boxes[0][indices_s[0]],
                                                                  preds[0]['boxes'][indices_t[0]], iou_threshold=0.5)
    ...
    for lab_index, box in enumerate(preds[0]['boxes']):
        cv2.rectangle(images[0], (int(box[0].item()), int(box[1].item())), (int(box[2].item()), int(box[3].item())),
                      color_dict[preds[0]['labels'][lab_index].item()], 3)

    cv2.imwrite('./Prop_foggy/' + str(index) + '.png', images[0])
